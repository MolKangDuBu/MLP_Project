...
<!-- 여기부터 리스트 추가 -->
   			...
   			<%
   			List<FoodReviewDto> list = (List<FoodReviewDto>)request.getAttribute("reviewList");
   			           		for(FoodReviewDto tempDto : list){
   			%>
...
function changeSearch(id){
	...
	frm.pg.value=0;
	frm.action = "<%=request.getContextPath()%>/foodreview/list";
	frm.method ="GET";
	frm.submit();
...
function goPage(pg){
	...
	frm.pg.value = pg;
	frm.method = "get";
	frm.action = "${pageContext.request.contextPath}/foodreview/list";
	frm.submit();
}
...
function goView(id){
	...
	frm.method = "get";
	frm.action = "${pageContext.request.contextPath}/foodreview/view";
	frm.submit();
}
</script>
